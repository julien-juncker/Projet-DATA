...
def create_data_model(population, xmax, ymax):
    data = {}
    data['distance_matrix'] = generate_matrix(population, xmax, ymax)
    data['num_vehicles'] = 4
    data['demands'] = get_weight(population)
    data['vehicle_capacities'] = [population, population, population, population]
    data['depot'] = 0   
    return data
...
def launch(population, xmax, ymax):
    """Solve the CVRP problem."""
    # Instantiate the data problem.
    data = create_data_model(population, xmax, ymax)
...
def main():
    res = result()
    file = open("resultweight.json","a")
    for j in range(190):
        temp = []
        for i in range(5):
            temp.append(launch(j + 10, 200, 200))
        res.onetruck.append(sum(temp) / len(temp))
        file.writelines(str(j+10))
        file.writelines(":" + str(res.onetruck[j]) + ":1\n")
        file.flush()
    file.close()
